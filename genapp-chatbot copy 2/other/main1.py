...
def process_message(question):
  ...
  # format_prompt=formatPrompt(llm_data['answer'])
  # format_response=LLM_MODEL.predict(format_prompt, **PARAMETERS)
  # format_data = json.loads(llm_response.text)
  # print(format_data)
...
##############
  ...
  # print(searchResults)
  return res, genapp_response, searchResults[:3], total_time
...
def handle_webhook():
  ...
  if validation != True : ##<----   *****
    ...
    if data['resource'] == "messages" and data['event'] == "created":
      ...
      response_json, genapp_answer, suggested_list, total_time = process_message(message_text)
