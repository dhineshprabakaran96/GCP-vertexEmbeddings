...
APIGEE_BOT_ACCESS_TOKEN = os.environ["BOT_ACCESS_TOKEN"]
...
def get_message(message_id):
  ...
  headers = {
      "Authorization": f"Bearer {APIGEE_BOT_ACCESS_TOKEN}"
  }
...
def process_message(question):
  ...
  url="https://discoveryengine.googleapis.com/v1alpha/projects/655678175973/locations/global/collections/default_collection/dataStores/apigee_1705998416929/conversations/-:converse"
  ...
  qs_prompt=question_prompt.question_prompt(question)
    ...
    if name in path:
      ...
      CARD_PAYLOAD["body"].append({
            ...
            "actions": [
                {
                    "type": "Action.OpenUrl",
                    "title": "Enterprise API Wiki | Home",
                    "url":"https://www.eesewiki.ford.com/display/FEAPIS/Enterprise+API+Wiki" ## replace this URL for suggestion
                }
            ],
            "horizontalAlignment": "Left",
            "spacing": "Small"
...
def send_message_web(question, response_json, format_response, suggested_list):
 ...
 # Use the title as the key and the URL as the value
          ...
          links[name] = "https://www.eesewiki.ford.com/display/FEAPIS/Enterprise+API+Wiki" 
      # print(links[title])
...
def get_conversation_id(room_id):
  ...
  if room_id in conversation_ids:
      ...
      url="https://discoveryengine.googleapis.com/v1/projects/ford-4360b648e7193d62719765c7/locations/global/collections/default_collection/dataStores/apigee_1705998416929/conversations"
...
def handle_webhook(data):
  ...
  if 'orgId' in data:
    ...
    if 'parentId' not in data['data'].keys():
      ...
      validation, validation_msg = validate_request.validate_request(request.get_data(), request.headers.get('X-Spark-Signature'))
      ...
      if validation!= True : ##<----   *****
        ...
        if data['resource'] == "messages" and data['event'] == "created":
          ...
          cdsid=sender_email
          if sender_email !=["bmc-genapp@webex.bot", "chatBlueFord@webex.bot"] :
