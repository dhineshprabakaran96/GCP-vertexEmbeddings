...
def upload_data_bq(question, format_response, response_json, cdsid, request_source, tstamp):
  ...
  answer=format_response if response_json['answer'] == "NA" else str(format_response.candidates[0])
      ...
      if validation== True : ##<----   *****
        ...
        if data['resource'] == "messages" and data['event'] == "created":
          ...
          if sender_email !="chatBlueFord@webex.bot" :
            ...
            upload_data_bq(message_text, format_response, response_json, sender_email,request_source, data['data']['created']) # Upload question-answer data to bq with unique sessionID
