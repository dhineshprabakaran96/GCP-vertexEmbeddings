...
def upload_data_bq(question, answer, cdsid, request_source, tstamp):
  ...
  answer=str(answer.candidates[0])
      ...
      if validation== True : ##<----   *****
        ...
        if data['resource'] == "messages" and data['event'] == "created":
          ...
          if sender_email !="chatBlueFord@webex.bot" :
            ...
            upload_data_bq(message_text, format_response,  sender_email,request_source, data['data']['created']) # Upload question-answer data to bq with unique sessionID
