...
feedback_bq_table = "astrobot_followup"
...
def get_followup(message_id, room_id, cdsid, conversation_ID, tstamp):
...
#get_followup response from GenAPp
  ...
  #GenApp follows up response for three variations of the questions
  qs_prompt=question_prompt(followup_question)
  ...
  print(genapp_reply)
  prompt =  construt_prompt(str(results), followup_question) ## 4. Construct prompt
  ...
  format_prompt=f"""Create a well-structured and readable block of the below text that explains a concept or process using clear and concise language. Pay attention to formatting, such as using bullet points, numbered lists, or headers, to improve the overall readability of the text.
  \n
  {llm_data['answer']}
  """
  ...
  CARD_PAYLOAD['body'][1]['text'] = format_response
...
def upload_data_bq(question, answer, cdsid, conversation_ID, tstamp, exec_time):
...
VALUES ('{sessionID}', '{cdsid}', '{question}', '{str(answer)}', '{round(exec_time,3)} seconds', '{tstamp}')"""
  ...
  try:
    client.query(query)
    print("Data written to BigQuery table successfully")
  except GoogleAPIError as e:
      print(f"Error writing data to BigQuery table: {e}")
...
def process_message(question):
  ...
  with ThreadPoolExecutor() as executor:
    futures = [executor.submit(process_query, query) for query in questions]
...
def send_message(room_id, response_json, format_response, genapp_answer, suggested_list):
  ...
  CARD_PAYLOAD['body'][1]['text'] = str(format_response)
...
def send_message_web(question,format_response, suggested_list):
  # print(suggested_list)
  ...
  for item in suggested_list:
      ...
      if name in path:
          links[name] = f"<a href='{path[name]}'>{name}</a>"  # Use the title as the key and the URL as the value
      else:
  ...
  if 'orgId' in data:
    ...
    if validation == True : ##<----   *****
      ...
      if data['resource'] == "messages" and data['event'] == "created":
        ...
        upload_data_bq(message_text, format_response,  sender_email, conversation_ID, data['data']['created'], total_time) # Upload question-answer data to bq with unique sessionID
    ...
    return send_message_web(question,format_response, suggested_list)
  else:
    return "Authentication failed!"
