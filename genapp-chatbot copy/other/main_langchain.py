...
def upload_data_bq(question, answer, cdsid, sessionID, conversation_ID, request_source, exec_time, tstamp ):
  ...
  query = f"""INSERT INTO `{PROJECT_ID}.chatgpt.{feedback_bq_table}` (session_id, cdsid, query, response, response_time, response_timestamp, conversation_id)
VALUES ('{sessionID}', '{cdsid}', '{question}', '{str(answer)}', '{round(exec_time,3)} seconds', '{tstamp}', '{conversation_ID}')"""
